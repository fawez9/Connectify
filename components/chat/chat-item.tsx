...
import { ShieldAlert, Edit, ShieldCheck, Trash, FileIcon } from "lucide-react";
...
const RoleIconMap = {
  GUEST: null,
  MODERATOR: <ShieldCheck className="h-4 <-4 ml-2 text-indigo-500" />,
  ADMIN: <ShieldAlert className="h-4 <-4 ml-2 text-rose-500" />,
};
...
export const ChatItem = ({ id, content, member, timestamp, fileUrl, deleted, currentMember, isUpdated, socketUrl, socketQuery }: ChatItemProps) => {
  ...
  const onMemberClick = () => {
    ...
    rounter.push(`/servers/${params?.serverId}/conversations/${member.id}`);
  };
    ...
    <div className="relative group flex items-center hover:bg-black/5 p-4 transition w-full">
      <div className="group flex gap-x-2 items-start w-full">
        ...
        <div className="flex flex-col w-full">
          ...
          {!fileUrl && isEditing && (
            <Form {...form}>
              <form className="fmex items-center w-full gap-x-2 pt-2" onSubmit={form.handleSubmit(onSubmit)}>
                <FormField
      ...
      {canDeleteMessage && (
        <div className="hidden group-hover:flex items-center gap-x-2 absolute p-1 -top-2 right-5 bg-white dark:bg-zinc-800 border rounder-sm">
          {canEditMessage && (
